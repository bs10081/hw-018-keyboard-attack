#!/bin/bash
# Zip Bomb Linux 版本專用編譯腳本
# 用於編譯和燒錄 zipbomb_linux.ino 到 Digispark

set -e

SKETCH_NAME="zipbomb_linux"
SKETCH_PATH="$(pwd)/${SKETCH_NAME}.ino"
BUILD_DIR="$(pwd)/build_zipbomb_linux"
...
upload() {
    ...
    if [ ! -f "$HEX_FILE" ]; then
        ...
        echo "請先執行: ./build_zipbomb_linux.sh compile"
        exit 1
    fi
...
show_help() {
    echo "Zip Bomb Linux 版本編譯腳本"
    echo ""
    echo "用法: $0 [指令]"
    echo ""
    echo "指令:"
    echo "  compile    編譯 zipbomb_linux.ino"
